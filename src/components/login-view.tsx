...
export default function LoginView({ onLogin, onBack, onSignup }: LoginViewProps) {
      ...
      >
        <div className="glass-panel rounded-2xl p-8 shadow-2xl border border-primary/20 backdrop-blur-xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold gold-gradient-text mb-2 font-serif">Welcome Back</h1>
            <p className="text-muted-foreground">Enter the Cave of Wonders</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Open Sesame"}
            </button>
          </form>

          <div className="mt-6 text-center text-sm text-muted-foreground">
            New to the kingdom?{" "}
            <button onClick={onSignup} className="text-primary hover:text-amber-300 transition-colors font-medium hover:underline">
              Create an account
            </button>
          </div>
        </div>
